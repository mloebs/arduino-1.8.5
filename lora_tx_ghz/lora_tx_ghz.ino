...
void loop()
{
  uint16_t packet_delay = 100 * 1; // 100 ms packet delay
  
  //set frequency based on channelsd
  uint8_t  current_channel = channel[random(0,81)];
  uint32_t freq = 1000000 * (2400 + current_channel);
  LT.setRfFrequency(freq, Offset);

  Serial.print("Ch:");
  Serial.print(current_channel);
  Serial.print(" ");
  //Serial.print("Freq:");
  //Serial.print(LT.getFreqInt());
  Serial.print(" ");
  ...
  buff[TXPacketL - 1] = ids[random(0,10)];                                   //replace null character at buffer end so its visible on reciver
...
void setup()
{
  ...
  //LT.setRfFrequency(Frequency, Offset); to loop
  ...
  for (int i = 0; i < 81; i++) //set 80 channels
{
  channel[i] = i;
}
}
